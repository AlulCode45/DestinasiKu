    ...
    <div class="bg-white shadow p-5 rounded-md">
        ...
        <form action="{{ route('vendor.update', $company->id) }}" method="POST">
            ...
            <textarea rows="6" placeholder="Description" name="description"
                class="w-full rounded-lg border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary">{{ $company->description }}</textarea>
            <div class="mb-5">
                <button type="submit"
